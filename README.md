...
> Windows 桌面 OCR + 情境化翻譯 + Azure TTS 朗讀 — 全程本地 VLM，離線可用。

按 **Win+Q** 框選螢幕任意區塊，本地視覺語言模型（VLM）單次 pass 完成「辨識 + 翻譯成目標語言」，結果視窗即時串流顯示，可一鍵複製或用 Azure 神經語音朗讀。
...
- **一個熱鍵直達翻譯**：框到什麼語言都行，一律翻成你設定的目標語言，OCR 與翻譯一次完成，不用先複製再貼到翻譯網站。
...
- **單次 pass OCR + 翻譯**：模型一次完成辨識與翻譯，串流輸出從一開始就是目標語言，沒有過場閃爍
...
「設定 → 語言」勾選要啟用的語言；「設定 → 翻譯」選目標語言（必須在啟用清單內）。注意各模型檔位的語言覆蓋不同（見上表）— 想用進階語言請選 9B。

## 翻譯行為

不論框到什麼語言，一律翻成你設定的**目標語言**（也可從 tray 選單快速切換目標語言）。如果原文本身已經是目標語言，模型會原文照出，此時就是純 OCR 抓字工具。
...
v0.6.0 對閒置資源佔用做了大幅優化（縮小 batch buffer + flash-attn 自動啟用）：開發機實測 llama-server 常駐 RAM 從 17.2 GB 降到 **2.9 GB（-83%）**、VRAM 再省約 2 GB，長文推論同時快 18–55%。模型仍是常駐設計（熱鍵響應只等推論時間），但閒置時的佔用已經從「很兇」降到「一個瀏覽器分頁群」的量級。

基本門檻：

- **OS**：Windows 10 / 11（x64）
- **RAM**：16 GB 建議（llama-server 常駐約 3 GB + 系統 + browser）
- **硬碟**：依模型 2–7 GB
...
模型在程式啟動時載入並常駐，熱鍵響應只等推論時間；閒置佔用約 3 GB RAM（4B 實測），比 v0.5.0 之前低一個數量級。
...
3. **翻譯** — 選目標語言；選情境（通用 / 航運 / 遊戲 / 程式碼 / 醫療）或自建
