...
### 資源佔用大幅下降（實測，開發機 4B 模型）
- **llama-server 常駐 RAM：17.2 GB → 2.9 GB（-83%）**（batch 1024 / ubatch 512 + flash-attn auto）
- **VRAM 再省約 2 GB**；英文長圖推論 9.2s → 4.1s（-55%）
- GPU vision offload 預設化後 CPU 尖峰消失，30 輪壓測零洩漏
